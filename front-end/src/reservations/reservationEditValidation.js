function reservationEditValidation(reservation) {
  const errors = [];
  const { reservation_date, reservation_time, people, reservation_id } =
    reservation;
  const props = [
    "first_name",
    "last_name",
  ...
  if (!reservation_id) {
    errors.push(new Error(`Please enter a reservation Id`));
  }
  return errors;
}

export default reservationEditValidation;
